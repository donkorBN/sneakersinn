...
function createProductCard(product) {
    ...
    const whatsappUrl = product.soldOut ? '#' : generateWhatsAppLink(product);
    const cardClass = product.soldOut ? 'product-card reveal sold-out' : 'product-card reveal';
    const buttonClass = product.soldOut ? 'btn btn-whatsapp btn-sm disabled' : 'btn btn-whatsapp btn-sm';
    const buttonText = product.soldOut ? 'Sold Out' : `
    ...
    return `
    <div class="${cardClass}" data-category="${product.category}">
      ${badge}
      ${product.soldOut ? '<div class="sold-out-overlay">SOLD OUT</div>' : ''}
      <a href="${product.soldOut ? '#' : `product.html?id=${product.id}`}" class="product-image-link">
        <img src="${product.image}" alt="${product.name}" class="product-image" loading="lazy" style="${product.soldOut ? 'filter: grayscale(100%); opacity: 0.7;' : ''}">
      </a>
      <div class="product-info">
        <h3 class="product-name">${product.name}</h3>
        <p class="product-price">${formatPrice(product.price)}</p>
        <div class="product-actions">
          <a href="${product.soldOut ? '#' : `product.html?id=${product.id}`}" class="btn btn-outline btn-sm ${product.soldOut ? 'disabled' : ''}">View Details</a>
